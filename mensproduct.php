...
if(isset($_POST['Filter_BTN']))
{
	...
	{	$Price = $_POST['Price_sort'];
		...
		}if($Price=="1500-2000")
		{
			...
			$sql="SELECT * FROM  products WHERE price>='$priceMin' AND price<='$priceMax'AND gender='0'";
}}
else if(isset($_POST['priceL_H']))
{
	$sql="SELECT * FROM  products WHERE gender='0' ORDER BY price";
}
else if(isset($_POST['priceH_L']))
{
	$sql="SELECT * FROM  products WHERE gender='0' ORDER BY price DESC";
}
else if(isset($_POST['nameL_H']))
{
	$sql="SELECT * FROM  products  WHERE gender='0' ORDER BY product";
}
else if(isset($_POST['nameH_L']))
{
	$sql="SELECT * FROM  products WHERE gender='0' ORDER BY product DESC";
}
else if(isset($_POST['discountL_H']))
{
	$sql="SELECT * FROM  products WHERE gender='0' ORDER BY Disc DESC ";
}
else
{
		$sql = "SELECT * FROM  products WHERE gender='0'";
}		
